...
int CompileLinkVertexFragmentShader(const char *vss, const char *fss) {
  ...
  if (!success) {
    ...
    std::cout << "Vertex shader compilation failed: " << shaderCompileInfoLog
              << std::endl;
  }
  ...
  if (!success) {
    ...
    std::cout << "Fragment shader compilation failed: " << shaderCompileInfoLog
              << std::endl;
  }
  ...
  if (!success) {
    ...
    std::cout << "Linking shader program failed: " << shaderLinkInfoLog
              << std::endl;
  }
  return id;
}
