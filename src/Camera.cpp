...
const glm::mat4 Camera::GetView() {
  ...
  glm::vec3 directiontopos(glm::vec4(0.0f, 0.0f, -1.0f, 1.0f) *
                           arcBallRotation);
  glm::vec3 upvector(glm::vec4(0.0f, 1.0f, 0.0f, 1.0f) * arcBallRotation);
  return glm::lookAt(
      (glm::normalize(directiontopos) * glm::vec3(zoom, zoom, zoom)) + position,
      position, glm::normalize(upvector));
}
glm::vec3 Camera::CalculateDirectionToMoveIn(float around, float updown) {
  glm::vec4 direction =
      glm::vec4(0.0f, 0.0f, -1.0f, 1.0f) * ArcBallRotation(around, updown);
  direction.y =
      0.0f; // remove y component, we are only interested in plane movement
