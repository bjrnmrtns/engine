...
static char const *vss = "#version 330 core\n"
...
Renderer::Renderer()
    : shaderProgramId(CompileLinkVertexFragmentShader(vss, fss)), modellocation(glGetUniformLocation(shaderProgramId, "model")),
      viewlocation(glGetUniformLocation(shaderProgramId, "view")), projectionlocation(glGetUniformLocation(shaderProgramId, "projection")),
      selectioncolorlocation(glGetUniformLocation(shaderProgramId, "selectioncolor")) {}

Renderer::~Renderer() { glDeleteProgram(shaderProgramId); }

void Renderer::Draw(Source &source, const glm::mat4 &model, const glm::mat4 &view, const glm::mat4 &projection,
                    float selectioncolor) const {
