...
void Source::BufferData(const ::Mesh::Vertex data[], int elements) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, vbo);
  glBufferData(GL_ARRAY_BUFFER, sizeof(::Mesh::Vertex) * elements, &data[0],
               GL_STATIC_DRAW);
}
...
Renderer::Renderer()
    : shaderProgramId(CompileLinkVertexFragmentShader(vss, fss)),
      ...
      selectioncolorlocation(
          glGetUniformLocation(shaderProgramId, "selectioncolor")) {}

Renderer::~Renderer() { glDeleteProgram(shaderProgramId); }

void Renderer::Draw(Source &source, const glm::mat4 &model,
