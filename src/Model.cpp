...
const ::Mesh::TStaticMesh Model::Grid() {
    ...
    for (int x = 0; x < 100; x++) {
        for (int z = 0; z < 100; z++) {
            if ((x + z) % 2 == 1) {
                ::Mesh::TStaticMesh element = Move(AdjustColor(oddCell, glm::vec3(xColorAdjustment * x, 0.0f, zColorAdjustment * z)),
                                                   glm::vec3(-100 + x * 2, -1.0f, -100 + z * 2));
                grid.insert(grid.end(), element.begin(), element.end());
            } else {
                ::Mesh::TStaticMesh element = Move(AdjustColor(evenCell, glm::vec3(xColorAdjustment * x, 0.0f, zColorAdjustment * z)),
                                                   glm::vec3(-100 + x * 2, -1.0f, -100 + z * 2));
                grid.insert(grid.end(), element.begin(), element.end());
            }
        }
