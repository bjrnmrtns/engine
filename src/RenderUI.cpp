...
void Source::BufferData(const ::UI::Vertex data[], int elements) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(::UI::Vertex) * elements, &data[0], GL_STATIC_DRAW);
}
...
Renderer::Renderer()
    : shaderProgramId(CompileLinkVertexFragmentShader(vss, fss)), projectionlocation(glGetUniformLocation(shaderProgramId, "projection")) {}
