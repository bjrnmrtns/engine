...
bool IsEntitySelected(const glm::vec3 &cornerA, const glm::vec3 &cornerB, const glm::vec3 &entityPosition) {
    ...
    return (xmin < entityPosition.x && entityPosition.x < xmax && zmin < entityPosition.z && entityPosition.z < zmax);
}
...
class Entity {
  public:
    ...
    Entity(const glm::vec3 &startPosition) : entityId(lastEntityId++), position(startPosition) {}
    int teamId = 0;
    int entityId;
    glm::vec3 position;
    bool selected = false;
...
int main(int argc, char *argv[]) {
    ...
    if (SCREEN_FULLSCREEN) {
        window = SDL_CreateWindow("demo", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 0, 0,
                                  SDL_WINDOW_FULLSCREEN_DESKTOP | SDL_WINDOW_OPENGL);
    } else {
        window = SDL_CreateWindow("demo", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_OPENGL);
    }
