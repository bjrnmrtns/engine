...
static bool MousePickTo3d(const glm::mat4 &view, glm::mat4 &proj, int width,
                          int height, int mousex, int mousey,
                          glm::vec3 &outValue) {
  glm::vec3 p0 = glm::unProject(glm::vec3(mousex, mousey, -1.0f), view, proj,
                                glm::vec4(0.0f, 0.0f, width, height));
  glm::vec3 p1 = glm::unProject(glm::vec3(mousex, mousey, 1.0f), view, proj,
                                glm::vec4(0.0f, 0.0f, width, height));
  glm::vec3 d = p1 - p0;
  float t = -p0.y / d.y;
  if (t > 0.0) {
...
void SendToServer(int tick, std::vector<Command> &toSend,
...
void ReceiveFromServer(std::vector<Command> &toReceive,
                       std::vector<Command> &serverCommandQueue) {
...
struct Selection {
  ...
  State state;
  // TODO: consider all 2d corners and convert all 4 to 3d and see what is
  // selected
...
bool IsEntitySelected(const glm::vec3 &cornerA, const glm::vec3 &cornerB,
...
public:
  ...
  Entity(const glm::vec3 &startPosition)
      : entityId(lastEntityId++), position(startPosition) {}
  int teamId = 0;
  int entityId;
  glm::vec3 position;
  bool selected = false;
...
// TODO: Real networking (out of sync detection -> hash game state (placement
// new?))
int main(int argc, char *argv[]) {
  ...
  if (SCREEN_FULLSCREEN) {
    window = SDL_CreateWindow(
        "demo", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 0, 0,
  ...
  glm::mat4 projectionM =
      glm::perspective(45.0f, width / (float)height, near, far);
  ...
  for (size_t i = 0; i < sizeof(Configuration::EntityDefinitions) /
  ...
  while (!quit) {
      ...
      case Command::Tick: {
        ...
        for (int i = 0; i < ticksTodo; i++) {
          for (auto it = entities.begin(); it != entities.end(); ++it) {
            if (!it->isAtTarget) {
              it->position +=
                  glm::normalize(it->targetPosition - it->position) *
                  (it->speed / stepFrequency);
    ...
    for (auto it = entities.begin(); it != entities.end(); ++it) {
      if (it->selected) {
        meshRenderer.Draw(tank, glm::translate(glm::mat4(1.0f), it->position),
                          camera.GetView(), projectionM, 1.0);
      } else {
        meshRenderer.Draw(tank, glm::translate(glm::mat4(1.0f), it->position),
                          camera.GetView(), projectionM, 0.5);
      }
    }
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_MOUSEMOTION: {
        if (!keys.leftctrl) {
          ...
          camera.MouseMove(event.motion.xrel / 100.0f,
                           -event.motion.yrel / 400.0f);
        }
        break;
      }
      case SDL_MOUSEBUTTONDOWN: {
        if (keys.t) {
          glm::vec3 target;
          if (MousePickTo3d(camera.GetView(), projectionM, width, height,
        ...
        if (!keys.t && selection.state == Selection::State::NotSelecting) {
          if (MousePickTo3d(camera.GetView(), projectionM, width, height,
                            mousex, mousey, selection.clicka3d)) {
      ...
      case SDL_MOUSEBUTTONUP: {
        if (selection.state == Selection::State::BeginSelection) {
          if (MousePickTo3d(camera.GetView(), projectionM, width, height,
                            mousex, mousey, selection.clickb3d)) {
            selection.state = Selection::State::EndSelection;
            selection.clickb = glm::vec2(mousex, mousey);
            for (auto it = entities.begin(); it != entities.end(); ++it) {
              if (IsEntitySelected(selection.clicka3d, selection.clickb3d,
                                   it->position)) {
                it->selected = true;
              } else {
                it->selected = false;
